...
def create_memory_usage_plot(algorithm, db_sizes):
    lines = []
    for _cfg in CFGs:
        data_values = []
        for db_size in db_sizes:
            filename = MEMORY_USAGE_FILENAME + "_" + "_".join(str(_str) for _str in _cfg) + f'_{db_size}.txt'
            data = np.loadtxt(f"{BENCHMARKS_INPUT_FOLDER}/{algorithm}/{filename}")
            data_values.append(sum(data))
